...
class AddOC extends StatefulWidget {
  int id_customer;
  AddOC({super.key, required this.id_customer});

  @override
  State<AddOC> createState() => _AddOCState();
}

class _AddOCState extends State<AddOC> {
  ...
  Future<void> loadData() async {
    ...
    day_inicio = DateFormat.d().format(now);
    month_inicio = DateFormat.M().format(now);
    year_inicio = DateFormat.y().format(now);
    day_fin = DateFormat.d().format(now);
    month_fin = DateFormat.M().format(now);
    year_fin = DateFormat.y().format(now);
    setState(() {
      isLoading = false;
    });
  }
  ...
  getCustomerAddress() async {
    List<CustomersClass> customer1 =
        await DataAccessObject.selectCustomer(widget.id_customer);
    setState(() {
      List splitCustomerName = customer1[0].name!.split(" ");
      prefijo.text = "${splitCustomerName[0]}_";
      pais.text = customer1[0].country!;
      estado.text = customer1[0].state!;
      ciudad.text = customer1[0].city!;
      cp.text = customer1[0].cp.toString();
      List separateStreet = customer1[0].street!.split(',');
...
// This is called Immediatelly when user clicks suggestion and BEFORE
  ...
  void onSuggestionClick(Place placeDetails) {
    ...
    setState(() {
      ...
      street.text = placeDetails.street!;
  ...
  Widget fieldAutocomppleteCP() {
    return // Postalcode lookup TextField example
        AddressAutocompleteTextField(
      //componentCountry: idCountry,
      language: 'en-Us',
  ...
  postData() async {
    if (_formKeyOC.currentState!.validate()) {
      ...
      // print(
      //     'id_customer = ${widget.id_customer}, Fecha inicio = $fecha_inicio, Fecha fin = $fecha_fin, Solicitante = ${solicitante.text}, country = $country, State = ${estado.text}, City = ${ciudad.text}, CP = ${int.parse(cp.text)}, Street = ${street.text}, Prioridad = $prioridad, Moneda = $moneda');
      try {
        result = await DataAccessObject.postOC(
            ordenCompra.text,
            widget.id_customer,
            fecha_inicio,
            fecha_fin,
