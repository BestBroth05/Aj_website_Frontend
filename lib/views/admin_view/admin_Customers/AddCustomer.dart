  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: TitleH1(context, "Add Customer"),
          leading: IconButton(
              onPressed: () {
...
// *********************** Search Ubication *********************** //
  ...
  postData() async {
    if (_formKeyCustomer.currentState!.validate()) {
      // List countrySplit = country.split(' ');

      // country = countrySplit[countrySplit.length - 1];
      // print(country);
      // print(
      //     '$name, $rfc, $contactPerson, $phone, $country, $state, $city, $cp, $street');
      print('logo chido: ${selectedImageInBytes.toString()}');
      int result = await DataAccessObject.postCustomer(
          name.text,
          email.text,
          rfc.text,
          contactPerson.text,
