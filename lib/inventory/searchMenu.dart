...
Category selectedCategory = Category.capacitors;

class InventorySearchMenu extends StatefulWidget {
  @override
  _InventorySearchMenu createState() => _InventorySearchMenu();
}

class _InventorySearchMenu extends State<InventorySearchMenu> {
  List<Widget> filters = [];
  ...
  Widget build(BuildContext context) {
    int searchBarFlex = countFiltersOptions(selectedCategory);
    // idController.text = searchBarFlex.toString() + ' ${20 - searchBarFlex}';

    unitValue = selectedCategory.unitValue;

    // print('${selectedCategory.name} flex: $searchBarFlex');
    return Container(
            ...
            child: Container(
              ...
              child: MouseRegion(
                ...
                child: DropdownButtonHideUnderline(
                  child: dropDownMenu<Category>(Category.values, 0),
                ),
              ),
            ),
          ),
                  ...
                  child: Padding(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        await areFiltersEmpty(selectedCategory);
                      },
                    ),
                  ),
                ),
  ...
  Expanded searchFilterFill(
    ...
    Expanded textField = Expanded(
      child: AutoSizeTextField(
        ...
        minFontSize: 1,
        maxFontSize: 30,
        onEditingComplete: () async => await areFiltersEmpty(selectedCategory),
      ),
    );
        ...
        onChanged: (dynamic newValue) async {
          setState(
            () {
              ...
              if (indexOfValue == 0) {
                selectedCategory = selections[indexOfValue];
              }
            },
          );
          if (indexOfValue == 0) {
  ...
  Widget getSearchFilterDropdown<T>(
    ...
    if (title.split(' ').length > 1) {
      ...
      // print('$toCompare ${selectedCategory.searchSpaces} ');
    } else {
      toCompare = title.toLowerCase();
    }

    if (selectedCategory.searchSpaces.contains(toCompare) == false) {
      return Container();
    }
  ...
  Future<void> areFiltersEmpty(Category category) async {
    ...
    if (isSearching) {
      ...
      List<Product> components =
          await searchProductsByCategoryInDatabase(category);
      inventoryState.setState(() {
        products = components;
      });
    }
